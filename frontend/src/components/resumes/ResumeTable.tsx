...
import { FileText, Phone, Mail, User, Trash2 } from "lucide-react";
...
export function ResumeTable({ resumes, onDelete, onView }: ResumeTableProps) {
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "parsed":
        return <Badge className="bg-green-500/10 text-green-500 border-green-500/20">Parsed</Badge>;
      case "error":
        return <Badge variant="destructive" className="bg-destructive/10 text-destructive border-destructive/20">Error</Badge>;
      default:
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-card/30 backdrop-blur-sm overflow-hidden shadow-sm">
      <Table>
        ...
        <TableBody>
          {resumes.map((resume) => (
            <TableRow key={resume.id} className="hover:bg-muted/30 transition-colors">
              <TableCell>
